...
export function RevisionGate({ run, plan }: { run: SceneRun; plan: RevisionPlan }) {
  ...
  const pending = plan.items.some((item) => ['verify', 'undecidable'].includes(item.disposition))
