...
import { consolidatedScene, type SceneKey, type SceneRun } from './types'
...
export function PatchWorkspace({ run, profiles }: { run: SceneRun; profiles: ModelProfile[] }) {
  ...
  const steps = consolidatedScene(run) ? [patchSteps[0]] : patchSteps.filter(step => step.key !== 'scene-dialogue-patch' || run.snapshot.dialogue_split)
...
function PendingPatchStatus({ run }: { run: SceneRun }) {
  ...
  if (patch.checked) return <p role="status" className="scene-notice">{checkedLabel(run)} Review continuity below before accepting this scene.</p>
...
function checkedLabel(run: SceneRun) {
  if (run.patch!.no_changes_required) return 'No patch check needed.'
  return run.patch!.check_kind === 'director' ? 'You selected this unified patch. Review its change log before acceptance.' : 'Selected changed-passage check passed.'
}
