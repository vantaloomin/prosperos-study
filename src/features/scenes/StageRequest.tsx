...
import { sceneWritingPurpose } from './types'
import { RequestWritingChoices } from '../writing/RequestWritingChoices'
...
export function StageRequest({ run, step, profiles, onStarted, targets = {} }: { run: SceneRun; step: SceneKey; profiles: ModelProfile[]; onStarted: (id: string) => void; targets?: { review_job_ids?: string[]; item_id?: string } }) {
  ...
  const [writing, setWriting] = usePersistent<WritingChoices>(`roleplay:scene-writing:${run.id}:${step}`, { style: 'inherit', recipe: 'inherit', variables: {} })
  const purpose = sceneWritingPurpose(step)
  ...
  const body = { expected_revision: run.revision, key: step, profile_ids: selected, ...targets, ...(purpose ? { writing } : {}), ...(step === 'scene-dialogue' && actors.enabled ? { dialogue_actors: actorInputs(actors) } : {}) }
  ...
  return <section className="scene-request form-stack"><details className="scene-profiles"><summary>{selected.length ? `${selected.length} comparison profiles` : 'Use assigned model · change or compare'}</summary><p className="subtle">Leave empty to inherit this step’s saved model. Select up to four profiles for a deliberate comparison with identical source material.</p>{profiles.map((profile) => <label className="check-row" key={profile.profile_id}><input type="checkbox" checked={selected.includes(profile.profile_id)} disabled={!selected.includes(profile.profile_id) && selected.length >= 4} onChange={() => toggle(profile.profile_id)} />{profile.display_name ?? profile.name}</label>)}</details>
    ...
    {purpose && <RequestWritingChoices storyId={run.snapshot.branch.story_id} recipePurpose={purpose} value={writing} onChange={value => { setWriting(value); setPreview(null) }} />}
...
function StageSources({ job }: { job: StagePreview['jobs'][number] }) {
  return <><p className="subtle">Style: {writingLabel(job.writing.style)}. Recipe: {writingLabel(job.writing.recipe)}.</p><small>Input allowance: {job.input_allowance.toLocaleString()} tokens per call. Maximum output: {job.output_limit.toLocaleString()} tokens per call. Provider cost is unknown.</small><StageInputs job={job} /></>
}

function StageInputs({ job }: { job: StagePreview['jobs'][number] }) {
  ...
  return <><small>~{job.estimated_input_tokens.toLocaleString()} input tokens · {job.source_count} sources</small><SourceMemoryCoverage memory={job.source_memory} /><details><summary>Inspect this stage’s exact request</summary><pre className="authoring-prose" tabIndex={0}>{job.instructions}</pre><pre className="authoring-prose" tabIndex={0}>{job.content}</pre></details></>
}

function writingLabel(value: StagePreview['jobs'][number]['writing']['style']) { return value ? `${value.name} · v${value.number}` : 'None' }
