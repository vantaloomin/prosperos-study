...
function PublicationExport({ storyId, book }: { storyId: string; book: Publication }) {
  ...
  const [prepared, setPrepared] = useState<{ docx_url: string; epub_url: string; html_url: string; revision: number; words: number } | null>(null)
  const prepare = () => action.run(async () => setPrepared(await api(`/stories/${storyId}/manuscript/exports`, { expected_revision: book.revision, html_contents: contents })))
  return <section className="book-publication"><h2>Ready for the next reader</h2><p>Export the saved manuscript in chapter and scene order. DOCX includes editable headings and page numbers. EPUB includes a linked table of contents. HTML is a single file for offline reading and printing in a browser.</p><p className="subtle">The files contain the selected prose and publication details. Author’s notes, bookmarks, private context and model records stay in the workspace.</p><ol>{book.chapters.map(chapter => <li key={chapter.id}>{chapter.title} <small>· {chapter.scenes.length} scenes</small></li>)}</ol><label className="check-row"><input type="checkbox" checked={contents} disabled={action.busy} onChange={event => { setContents(event.target.checked); setPrepared(null) }} />Include a linked contents list in HTML</label><button className="button primary" disabled={action.busy || !book.words} onClick={prepare}>{action.busy ? 'Preparing…' : 'Prepare publication files'}</button><ErrorNotice message={action.error} />{prepared && <div className="book-downloads" role="status"><p>Prepared version {prepared.revision} · {prepared.words.toLocaleString()} words</p><a className="button" href={prepared.docx_url} download>Download DOCX</a><a className="button" href={prepared.epub_url} download>Download EPUB</a><a className="button" href={prepared.html_url} download>Download HTML</a></div>}</section>
}
