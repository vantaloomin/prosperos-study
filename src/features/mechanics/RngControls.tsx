...
export function RngControls({ branch, data, onSaved }: { branch: Branch; data: MechanicsContext; onSaved: () => void }) {
  ...
  return <div className="form-stack"><label className="mechanic-master check-row"><input type="checkbox" checked={settings.enabled} onChange={(e) => patch({ enabled: e.target.checked })} /><span>Enable automatic randomness<small>Off by default. Explicit Roll now and isolated table previews remain available.</small></span></label>
