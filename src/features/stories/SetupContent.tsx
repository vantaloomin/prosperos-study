...
export function SetupStory({ draft, patch }: { draft: SetupDraft; patch: (next: Partial<SetupDraft>) => void }) {
  return <div className="form-stack"><Field label="Story title" value={draft.title} maxLength={120} placeholder="The observatory" onChange={(event) => patch({ title: event.target.value })} hint="The only required creative detail. Everything else can grow later." /><StoryStyle value={draft} onChange={patch} /><TextField label="The opening situation" rows={4} value={draft.premise} maxLength={30000} onChange={(event) => patch({ premise: event.target.value })} placeholder="A place, a person, an unfinished question…" hint="Guidance for the writer. This is not added to the story history." />
