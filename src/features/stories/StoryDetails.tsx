...
export function StoryDetails({ story, branchId, onClose, onOpen }: { story: Story; branchId: string; onClose: () => void; onOpen: (selection: Selection) => void }) {
  ...
  const save = () => action.run(async () => {
    await api(`/stories/${story.id}`, { title, premise, archived, settings, expected_revision: story.revision }, 'PUT')
    onClose()
  })
  return <><Modal open onClose={onClose} title="Story details" description="A title, a direction, and room to grow."><div className="dialog-body form-stack"><Field label="Title" value={title} onChange={(e) => setTitle(e.target.value)} maxLength={120} /><TextField label="Story guidance" rows={8} value={premise} onChange={(e) => setPremise(e.target.value)} hint="Guidance shapes future writing; it does not rewrite past messages." /><details className="advanced-settings" onToggle={(event) => setPreferences(event.currentTarget.open)}><summary>Writing preferences</summary>{preferences && <Suspense fallback={<Loading label="Opening preferences…" />}><StoryPreferences value={settings} onChange={setSettings} /></Suspense>}</details><label className="check-row"><input type="checkbox" checked={archived} onChange={(e) => setArchived(e.target.checked)} />Archive this story</label><button className="button" onClick={() => setExporting(true)}>Export transcript</button><button className="button" onClick={() => setArchiving(true)}>Private archive & recovery</button><ErrorNotice message={action.error} /></div><footer className="dialog-footer"><span className="subtle">Archived stories can be restored.</span><button className="button primary" disabled={action.busy || !title.trim()} onClick={save}>Save details</button></footer></Modal>
