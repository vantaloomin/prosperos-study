...
interface Props { branches: BranchSummary[]; selected: string; onSelect: (id: string) => void; onClose: () => void; openedAt: number }
const ROW_HEIGHT = 80
const OVERSCAN = 4
...
function BranchTree({ rows, selected, onSelect, openedAt }: { rows: BranchRow[]; selected: string; onSelect: (id: string) => void; openedAt: number }) {
  ...
  return <div ref={element} className="branch-tree-scroll" role="tree" tabIndex={0} aria-label="Branch map" aria-activedescendant={focusedId ? `branch-tree-${focusedId}` : undefined} onKeyDown={keyboard} onScroll={(event) => setTop(event.currentTarget.scrollTop)}>
    <div className="branch-tree-canvas" style={{ height: Math.max(140, rows.length * ROW_HEIGHT) }}>
      {rows.slice(start, end).map((row, offset) => <BranchTreeRow key={row.branch.id} row={row} index={start + offset} selected={selected} focused={rows[focused]?.branch.id === row.branch.id} onSelect={onSelect} />)}
      {!rows.length && <p className="subtle branch-empty">No branches match this name.</p>}
    </div>
  </div>
}

function BranchTreeRow({ row, index, selected, focused, onSelect }: { row: BranchRow; index: number; selected: string; focused: boolean; onSelect: (id: string) => void }) {
  ...
  return <div className="branch-tree-row" style={{ top: index * ROW_HEIGHT, paddingLeft: `calc(16px + min(${indent}px, 12vw))` }}>
    ...
    <button id={`branch-tree-${row.branch.id}`} role="treeitem" tabIndex={-1} aria-level={row.depth + 1} aria-posinset={row.position} aria-setsize={row.siblings} aria-selected={row.branch.id === selected} data-focused={focused} className="branch-tree-node" onClick={() => onSelect(row.branch.id)}>
      <GitBranch size={16} /><span>{row.branch.name}<small>{row.depth ? `Depth ${row.depth.toLocaleString()} · from ${row.parentName}` : 'Where the story began'}</small></span>{row.branch.id === selected && <Check size={16} />}
    </button>
  </div>
}

export function BranchMap({ branches, selected, onSelect, onClose, openedAt }: Props) {
  const [search, setSearch] = useState('')
  const rows = useMemo(() => branchRows(branches), [branches])
  const filtered = useMemo(() => rows.filter((row) => row.branch.name.toLocaleLowerCase().includes(search.trim().toLocaleLowerCase())), [rows, search])
  ...
  return <Modal open onClose={onClose} title="Every path stays with you" description="Choose a branch to return to its own history and pinned library versions." wide>
    ...
    <BranchTree key={search} rows={filtered} selected={selected} onSelect={select} openedAt={openedAt} />
    <footer className="branch-map-help"><span>↑ ↓ explore · ← parent · → child · Enter opens</span><span>Deep paths show their exact depth and parent.</span></footer>
  </Modal>
}
