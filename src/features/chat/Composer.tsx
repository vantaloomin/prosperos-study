...
export function Composer({ branch, mode, transfer, onTransferred, onRandomness }: Props) {
  ...
  const action = useSubmission(branch, onSubmitted, clear)
  ...
  const send = () => {
    ...
    return action.send({ text, role, opportunity: useBeat ? prepared!.id : null, continue: autoContinue && canGenerate })
  }
...
function MessageModes({ mode, role, onChange }: { mode: StoryMode; role: Role; onChange: (value: Role) => void }) {
  const main = mode === 'roleplay' ? 'user' : 'narrator'
  const extras = composerOptions(mode).filter(([value]) => value !== main && value !== 'ooc')
  return <div className="composer-modes" aria-label="Writing mode"><button type="button" aria-pressed={role === main} onClick={() => onChange(main)}><PenLine size={14} />{mode === 'roleplay' ? 'Your character' : 'Story text'}</button><button type="button" aria-pressed={role === 'ooc'} onClick={() => onChange('ooc')}>{mode === 'roleplay' ? 'Out of character' : "Author’s note"}</button><select aria-label="More writing modes" value={extras.some(([value]) => role === value) ? role : ''} onChange={event => { if (event.target.value) onChange(event.target.value as Role) }}><option value="">More writing modes</option>{extras.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div>
}
