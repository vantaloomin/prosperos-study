...
interface Assessment {
  ...
  snapshot: { branch: { id: string; name: string }; writer_profiles: { name: string }[] }
}
...
function AssessmentContent({ run, onWriter }: { run: Assessment; onWriter: (id: string) => void }) {
  ...
  return <><p className="subtle">{run.snapshot.branch.name} · Assessment requests: {run.jobs.length}. Writer requests: {run.snapshot.writer_profiles.length}. {run.jobs.length > 1 ? 'Choose one report to continue.' : 'One validated report starts the saved writer request automatically.'}</p>
    ...
    {!!run.stopped && !run.generation_id && <p role="status">Assessment stopped. Choose explicitly when you are ready to continue.</p>}
...
function AssessmentReport({ job, run, busy, onChoose }: { job: AssessmentJob; run: Assessment; busy: boolean; onChoose: () => void }) {
  ...
  return <section className="candidate-view assessment-report"><div className="candidate-meta"><strong>{job.snapshot.profile.name}</strong><span role="status">{job.status} · attempt {job.attempt}</span></div>
    ...
    {!run.generation_id && !run.stale && <AssessmentJobAction job={job} busy={busy || action.busy} onChoose={onChoose} onRetry={() => void retry()} />}
    {run.selected_job_id === job.id && <p className="subtle">Used for the saved writer request.</p>}
...
function AssessmentJobAction({ job, busy, onChoose, onRetry }: { job: AssessmentJob; busy: boolean; onChoose: () => void; onRetry: () => void }) {
  if (job.status === 'done') return <button className="button primary" disabled={busy} onClick={onChoose}>Use this assessment & write</button>
