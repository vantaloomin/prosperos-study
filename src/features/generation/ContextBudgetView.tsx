...
export function ContextBudgetView({ budget }: { budget: ContextBudget }) {
  ...
  const total = budget.estimated_input_tokens + budget.output_tokens
  return <article className={`context-budget ${budget.fits ? '' : 'context-over-budget'}`}>
    ...
    <div className="context-budget-meter" role="meter" aria-label={`${budget.name} estimated context usage`} aria-valuemin={0} aria-valuemax={budget.context_tokens} aria-valuenow={Math.min(total, budget.context_tokens)} aria-valuetext={`${number(total)} estimated tokens including output, of ${number(budget.context_tokens)} configured`}>
      <span style={{ width: `${inputWidth}%` }} /><span style={{ width: `${outputWidth}%` }} /></div>
    <dl><div><dt>Input estimate</dt><dd>{number(budget.estimated_input_tokens)}</dd></div><div><dt>Output reserved</dt><dd>{number(budget.output_tokens)}</dd></div><div><dt>Configured limit</dt><dd>{number(budget.context_tokens)}</dd></div></dl>
