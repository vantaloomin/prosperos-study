    ...
    for row in rows:
        for path, markdown in decode(row['conversion'])['files'].items():
            if path.startswith('lorebook/entries/'):
                ...
                title = fields.get('name') or fields.get('comment') or f"Entry {metadata['source_index'] + 1}"
...
def import_entry(database, version_id, import_id, path):
    ...
    keyword_activation = bool(supported_keys) and not fields.get('constant')
    ...
    return {'entry': entry.model_dump(), 'original_fields': fields,
            'notice': 'Added entries start off. Only plain primary keywords are proposed; review all native rules before enabling. '
